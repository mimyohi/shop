...
import {
  userHealthConsultationsQueries,
...
function ProfileContent() {
  ...
  const { data: orders = [], isLoading: ordersLoading } = useQuery({
    ...ordersQueries.byEmail(user?.email || ""),
    enabled: !!user?.email,
  });
  ...
  const {
    data: savedHealthConsultation,
    isLoading: healthConsultationLoading,
  } = useQuery({
  ...
  const { data: pointsHistory = [], isLoading: historyLoading } = useQuery({
    ...pointsQueries.history(user?.id || "", 100),
    enabled: !!user?.id,
  });
